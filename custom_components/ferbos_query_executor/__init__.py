...
async def _run_sqlite_query(hass: HomeAssistant, args: dict) -> dict:
    ...
    if not query:
        return {
            "success": False,
            "error": {"code": "invalid", "message": "Missing query"},
        }
    ...
    if not db_path.exists():
        return {
            "success": False,
            "error": {"code": "not_found", "message": f"DB not found: {db_path}"},
        }
    ...
    try:
        async with aiosqlite.connect(db_path.as_posix()) as db:
            ...
            if is_select:
                ...
                return {
                    "success": True,
                    "data": {"rowcount": rowcount, "lastrowid": lastrowid},
                }
    except Exception as exc:
        return {
            "success": False,
            "error": {"code": "sqlite_error", "message": str(exc)},
        }
